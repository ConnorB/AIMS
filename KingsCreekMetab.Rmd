...
```
Coordinates for kings creek from neon site
```{r}
Site <- c("King")
...
<!-- Lets get DO data from NEON for kings creek, only run when downloading new data. -->
<!-- ```{r} -->
...
```{r}
waq_instantaneous <- readTableNEON("~/Dropbox/AIMS/KingsCreek/NEON/filesToStack20288/stackedFiles/waq_instantaneous.csv", "~/Dropbox/AIMS/KingsCreek/NEON/filesToStack20288/stackedFiles/variables_20288.csv")
PARWS_5min <- readTableNEON("~/Dropbox/AIMS/KingsCreek/NEON/filesToStack20042/stackedFiles/PARWS_5min.csv", "~/Dropbox/AIMS/KingsCreek/NEON/filesToStack20042/stackedFiles/variables_20042.csv")
TSW_5min <- readTableNEON("~/Dropbox/AIMS/KingsCreek/NEON/filesToStack20053/stackedFiles/TSW_5min.csv", "~/Dropbox/AIMS/KingsCreek/NEON/filesToStack20053/stackedFiles/variables_20053.csv")
...
BP_1min <- readTableNEON("~/Dropbox/AIMS/KingsCreek/NEON/filesToStack00004/stackedFiles/BP_1min.csv", "~/Dropbox/AIMS/KingsCreek/NEON/filesToStack00004/stackedFiles/variables_00004.csv")
...
```{r}
kingYSI <- dplyr::select(waq_instantaneous, startDateTime, dissolvedOxygen, dissolvedOxygenFinalQF, horizontalPosition, specificConductance, specificCondFinalQF) %>% 
...
kingPAR <- dplyr::select(PARWS_5min, startDateTime, PARMean, horizontalPosition, PARFinalQF) %>% 
  dplyr::rename(Light = PARMean) %>% 
  dplyr::rename(DateTime = startDateTime) %>% 
  dplyr::filter(horizontalPosition == "101") %>% 
  dplyr::filter(PARFinalQF == "0")

kingTemp <- dplyr::select(TSW_5min, startDateTime, surfWaterTempMean, finalQF, horizontalPosition) %>% 
...
kingBP <- dplyr::select(BP_1min, startDateTime, corPres) %>% 
  dplyr::rename(BP = corPres) %>% 
  dplyr::rename(DateTime = startDateTime)

kingBP$BP <- kingBP$BP * 10 # convert from kilkopascals to mb
...
rm(waq_instantaneous, PARWS_5min, TSW_5min, csd_continuousDischarge, BP_1min, WindSpeed)
```

Quickly plot everything
```{r}
ggplot(kingYSI, aes(DateTime, DO.obs)) +
  geom_point() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  theme_bw()

ggplot(kingYSI, aes(DateTime, specificConductance)) +
  geom_point() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  theme_bw()

ggplot(kingPAR, aes(DateTime, Light)) +
  geom_point() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  theme_bw()

ggplot(kingTemp, aes(DateTime, Temp)) +
  geom_point() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  theme_bw()

ggplot(kingQ, aes(DateTime, Q)) +
  geom_point() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  theme_bw()

ggplot(kingBP, aes(DateTime, BP)) +
  geom_point() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  theme_bw()

ggplot(KonzaWind, aes(DateTime, wnd)) +
  geom_point() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  theme_bw()
```
...
```{r}
rawDAT <- kingYSI %>%
  full_join(kingBP, by = "DateTime") %>%
  full_join(kingQ, by = "DateTime") %>% 
  full_join(kingTemp, by = "DateTime") %>% 
  full_join(KonzaWind, by = "DateTime")
```
...
date_seq <- seq(as.POSIXct("2016-12-07 18:05:00", tz = "UTC"),
  as.POSIXct("2021-07-31 23:50:00", tz = "UTC"),
  by = ("5 min")
)
...
min.fill$solar.time <- calc_solar_time(min.fill$DateTime, longitude = (LatLong$Long[LatLong$Site =="King"])) 
```
...
#rawDAT.DAT$CalcLight <- calc_light(all.DAT$solar.time, latitude = (LatLong$Lat[LatLong$Site == "King"]), longitude =  (LatLong$Long[LatLong$Site == "King"]), max.PAR = u(2326, "umol m^-2 s^-1"), attach.units = is.unitted(all.DAT$solar.time))

#all.DAT <- all.DAT %>% mutate(MergeLight = coalesce(CalcLight, lag(Light)))

# ggplot(all.DAT, aes(DateTime, CalcLight)) +
...
```{r}
# 1. DO
interpDO <- approx(x = x, y = a, xout = 1.473e+09:1.628e+09 , method = "linear", rule = 2:2) # xout - estimated from summary(rawDAT$Unix2) above
...
# 2. Water Temp
interpTemp <- approx(x = x, y = b, xout = 1.473e+09:1.628e+09 , method = "linear", rule = 2:2)
plot(rawDAT$Unix2, rawDAT$Temp)
points(interpTemp$x, interpTemp$y, col = "blue", type = "l")
rm(b)
gc()
# 3. BP
interpBP <- approx(x = x, y = c, xout = 1.473e+09:1.628e+09 , method = "linear", rule = 2:2)
plot(rawDAT$Unix2, rawDAT$BP)
points(interpBP$x, interpBP$y, col = "blue", type = "l")
rm(c)
gc()
# 4. Q
interpQ <- approx(x = x, y = d, xout = 1.473e+09:1.628e+09 , method = "linear", rule = 2:2)
plot(rawDAT$Unix2, rawDAT$Q)
points(interpQ$x, interpQ$y, col = "blue", type = "l")
rm(d)
gc()

interpWND <- approx(x = x, y = e, xout = 1.473e+09:1.628e+09 , method = "linear", rule = 2:2)
plot(rawDAT$Unix2, rawDAT$wnd)
points(interpWND$x, interpWND$y, col = "blue", type = "l")
rm(e, x, date_seq)
gc()
```
...
```{r}
DOinterp <- data.frame(Unix2 = interpDO$x, interpDO$y, interpTemp$y, interpTemp$y, interpBP$y, interpQ$y, interpWND$y)
...
```{r}
interpDAT <- data.frame(DateTime = oxy1$DateTime, solar.time = oxy1$solar.time, DO.obs = oxy1$interpDO.y, Temp = oxy1$interpTemp.y, DATE = oxy1$DATE, Q = oxy1$interpQ.y, BP = oxy1$interpBP.y, wnd = oxy1$interpWND.y)
...
LakeMetabK <- dplyr::select(interpDAT, DATE, wnd)
LakeMetabK <- LakeMetabK %>% group_by(DATE) %>%
  dplyr::summarise(wnd = mean(wnd)) %>% 
  dplyr::rename(datetime = DATE)

u10 <- wind.scale.base(LakeMetabK$wnd, 3)
LakeMetabK<- rmv.vars(LakeMetabK, 'wnd', ignore.offset=TRUE)


LakeMetabK <- data.frame(datetime = LakeMetabK, wnd = u10)

#KonzaWind10 <- dplyr::rename(KonzaWind10, wnd = y)
#KonzaWind10 <- dplyr::rename(KonzaWind10, datetime = x)


LakeMetabK <- k.cole(LakeMetabK)

LakeMetabK <-  LakeMetabK %>%  dplyr::rename(K600.daily = k600)

hist(LakeMetabK$K600.daily)
hist(log(LakeMetabK$K600.daily))
GasLog <- log(LakeMetabK$K600.daily)
```
...
AJU's DO sat func.
<!-- Calculate DO sat using DO and corrected BP -->
<!-- ```{r} -->
...
<!-- all.DAT$DO.sat <- osat(temp = all.DAT$Temp, bp = all.DAT$BP) # adding a column of DO sat based on temp and BP to the data.frame -->

<!-- # plotting DO sat by date to check the data -->
<!-- ggplot(interpDAT) + -->
<!--   geom_line(aes(DateTime, DO.sat, col = "DO saturation")) + -->
<!--   geom_line(aes(DateTime, DO.obs, col = "DO observed")) + -->
<!--   theme(axis.text.x = element_text(angle = 90, hjust = 1)) + -->
<!--   scale_x_datetime(labels = date_format("%m/%d/%y %H:%M")) + -->
<!--  # ylab(expression(paste(italic("DO"), " (mg L"^{ -->
<!--   #  -1 -->
<!--  # }, ")"))) + -->
<!--   theme_bw() + -->
<!--   theme(axis.text = element_text(size = 10), axis.title = element_text(size = 12), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.text = element_text(size = 8)) -->
...
```{r}
interpDAT$CalcLight <- calc_light(interpDAT$solar.time, latitude = (LatLong$Lat[LatLong$Site == "King"]), longitude =  (LatLong$Long[LatLong$Site == "King"]), max.PAR = u(2326, "umol m^-2 s^-1"), attach.units = is.unitted(interpDAT$solar.time))
...
rm(dat_group, dat_rm2, DOinterp, interpDAT, kingBP, kingPAR, kingQ, kingTemp, kingYSI, min.fill, oxy1, rawDAT)
...
mle_fit <- streamMetabolizer::metab(mle_specs, data = dat)
# mle_fit
```
...
```{r}
mle_fit2 <- streamMetabolizer::metab(mle_specs, data = dat, data_daily = nightk) # running with night-time k estimates
...
bayes_specs <- revise(bayes_specs, burnin_steps=1000, saved_steps=1000, n_cores=4, pool_K600='none', K600_daily_meanlog = 0.2359578)
...
BayesKing <-get_params(mm, uncertaintiy = 'ci') %>% dplyr::select(date, GPP.daily, GPP.daily.sd, ER.daily, ER.daily.sd, K600.daily, K600.daily.sd)

fwrite(BayesKing, "~/Dropbox/AIMS/KingsCreek/KingBaysAvgLakeK.csv")
```

Read in bays run
```{r}
#BayesKing <- fread("~/Dropbox/AIMS/KingsCreek/KingBays.csv")
```
...
BayesKing$NEP <- BayesKing$GPP.daily + BayesKing$ER.daily
```
...
points(met_preds3$date, met_preds3$GPP, col = 'red', pch = 1  )
points(met_preds4$date, met_preds4$GPP, col = 'blue', pch = 3)
points(BayesKing$date, BayesKing$GPP.daily, col = 'darkgreen', pch=5)
legend('topleft', pch = c(0, 1, 3, 5), col = c('black', 'red', 'blue', 'darkgreen'), c('night-time k', 'nlm k', 'smooth k', 'Bayes'))

plot(met_preds2$date, met_preds2$ER, pch = 0, ylim = c(-20, 0), ylab = 'ER \n(g O2 m-2 d-1)', xlab = "")
points(met_preds3$date, met_preds3$ER, col = 'red', pch = 1)
points(met_preds4$date, met_preds4$ER, col = 'blue', pch = 3)
points(BayesKing$date, BayesKing$ER.daily, col = 'darkgreen', pch = 5)
...
points(BayesKing$date, BayesKing$NEP, col = 'darkgreen', pch = 5)
abline(a = 0, b = 0)
```
...
GPPplot <- ggplot()+
              ...
              ylab('GPP \n(g O2 m-2 d-1)')+
              geom_point(data = BayesKing, aes(x = date, y = GPP.daily), shape = 5, color = "darkgreen")+
...
ERplot <- ggplot()+
              ...
              axis.line.x = element_blank())+
               labs(y = expression("g O"[2]*" m"^{-2}* " d"^{-1}))+
              ylab('ER \n(g O2 m-2 d-1)')+
              geom_point(data = BayesKing, aes(x = date, y = ER.daily), shape = 5, color = "darkgreen")+
...
NEPplot <- ggplot()+
              ...
              geom_point(data = BayesKing, aes(x = date, y = NEP), shape = 5, color = "darkgreen")+
...
```{r}
BayesKing <- BayesKing[BayesKing$K600.daily < 30, ]
BayesKing <- BayesKing[BayesKing$ER.daily > -30, ] # Probably not real days



plot(ER.daily~K600.daily, data = BayesKing, ylab = 'ER g O2 m-2 d-1', xlab = 'K600 d-1', pch = 20, col = 'aquamarine4')
summary(lm(ER.daily ~ K600.daily, data = BayesKing))
```
...
ggplot(BayesKing, aes(GPP.daily, ER.daily))+
...
BayesKing <- datefill(BayesKing)



ARGPP <- ggplot(BayesKing, aes(x = date))+
...
ARER <- ggplot(BayesKing, aes(x = date))+
...
ARNEP <- ggplot(BayesKing, aes(x = date))+
  geom_line(aes(y = NEP))+
...
annotate_figure(ARStacked, top = "Kings Creek (NEON)")
```
